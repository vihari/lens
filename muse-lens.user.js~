...
(function() { // anon. wrapper function
	...
	if(!DEBUG){
	    ...
	    {
		$_('#muse-status').html('Already highlighted!');
...
.muse-NER-name { border-bottom: 1px red dotted; }';
	...
	if(DEBUG)
	    window.textNodesOnPage = addTextNodes(document.body, null, true, ignore_invisible);
	else
	...
	function voteUp(o) {
		...
		{url: scoring_url, 
		 ...
		 error: function(response) { LOG ('WARNING: error voting up term: ' + value); }
		});
	}
